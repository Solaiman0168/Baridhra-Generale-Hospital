...
<?php $table_heading = "Galary";?>
<?php include 'include/sidebar.php';?>
<?php include 'include/body-top.php';?>
  
<?php
        $tbl_name="galary_details";       //your table name
        $targetpage = "galary_details.php";   //your file name  (the name of this file)
    ...
    {
        $ID = $_GET['delete'];
        $sql = "UPDATE $tbl_name SET `is_deleted` = 1 ,`deleted_by` = '$user_no', `deleted_on` = '$CURR_TIME' WHERE galary_details_id = $ID";
    ...
    {
            $galary_details_id = $_POST['galary_details_id'];
            $title = trim($_POST['title']);
            $details = trim($_POST['details']);
           
           
            
       $SQL = "SELECT * FROM $tbl_name WHERE `is_deleted` = 0 AND `title` ='$title' AND `details` = '$details' AND `galary_details_id` != '$galary_details_id'";
            ...
            if($COUNT < 1): 
                 if ($_FILES["image_url"]["error"] > 0) {
                  ...
                  $sql = "UPDATE $tbl_name SET `title` ='$title' , `details` = '$details' , `image_url` = '$image_url' WHERE `galary_details_id` = '$galary_details_id'";
                $result = mysqli_query($con,$sql);
                if($result)
                {
...
?>
    ...
    <?php
        if(isset($_GET['edit'])):
        $id = $_GET['edit'];
        $sql = "SELECT * FROM $tbl_name WHERE `galary_details_id` = $id ";
    ...
    ?>
     ...
     <div class="form-group " <?php if($mgs=='')echo "style='display:none;'" ?>>
            ...
            <div>
                <input type="hidden" name="galary_details_id" value="<?=$result['galary_details_id']?>" />
            </div>
        </div>
        ...
        <?php $i=$page*$limit-$limit+1; while($row = mysqli_fetch_array($result)):?>
            <tr>
               ...
               <td><a class="btn btn-primary" target="_blank" href="upload/<?=$row['image_url']?>"><i class="fa fa-eye" aria-hidden="true"></i> View</a></td>
                ...
                <td>
                    <a onclick="return confirm('Are you Sure Want to Edit?');" href="<?=$targetpage.'?edit='.$row['galary_details_id']?>" class="btn btn-primary"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
                    <a onclick="return confirm('Are you Sure Want to Delete?');" href="<?=$targetpage.'?delete='.$row['galary_details_id']?>" class="btn btn-danger"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
                </td>
            </tr>
        <?php endwhile;?>
        </table>
